...
def minimizer(problem, qubits, entanglement, layers, method, name,
    ...
    _, _, reprs = problem_generator(problem, qubits, layers,
    ...
    f = fidelity_minimization(quantum_context, train_data, reprs)
...
#     ax[1].scatter(laea_x(angles[:, 1], angles[:, 0]), laea_y(angles[:, 1], angles[:, 0]), c=sol_test[:, -1],
#                   cmap=colors_rightwrong, s=2, norm=norm_rightwrong)
#     plt.show()
